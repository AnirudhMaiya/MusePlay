...
$query = "SELECT artist_name,artist_image,artist_country,album_name
	...
	WHERE E_album_id6  = album_id and E_artist_id6 = artist_id and artist_id = 'edsheeran1';";
	
$query2 = "SELECT artist_name,COUNT(artist_name) as Most_Followed
	FROM userg,artist,user_follows_artist
	WHERE userg.email_id = user_follows_artist.E_mail_id5 and artist.artist_id = user_follows_artist.E_artist_id5 and artist_id= 'edsheeran1'
	GROUP BY (artist_name);";

$query3 = "SELECT artist_name,style
	FROM artist,artist_style 
	WHERE artist.artist_id = artist_style.artist_id7 and artist_id = 'edsheeran1';";
...
$email_id = $_SESSION['email_id'];
  ...
  while($row5 = pg_fetch_row($result5)) {
    ...
    <td style = "padding :50px;width:100px;"><a href = "<?php echo "albums.php?album_id=".$xyz."&artist_id="."edsheeran1" ; ?>"><img src="<?php echo $row5[1];?>" STYLE="WIDTH:250px; HEIGHT:250px;border-radius:10%;"/></a><?php echo $row5[0];?> </td>
	    <?php if ($count % 3 == 2){
		echo '</tr>';
		}
    $count++;
  ...
  <?php 
$email_id = $_SESSION['email_id'];
$query2 = pg_query("SELECT E_artist_id5	FROM user_follows_artist WHERE E_mail_id5 = '$email_id' AND E_artist_id5 ='edsheeran1'"); 	
...
<?php } ?>
	<div class = "bg-centred" style = "width:240px;top:420px;left:420px;font-size:25px;"><a href = "<?php echo "bio.php?album_id=album_edsheeran1"."&artist_id=edsheeran1" ; ?>"><font face="Poppins-Regular">Jump to Artist's Bio</font></a></div> 
